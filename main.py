...
@app.post("/api/test/simulate-agent")
async def simulate_agent_data(test_request: dict):
    """Simulate agent data for beta testing purposes"""
    try:
        global latest_scan_data, latest_alerts_data
        
        email = test_request.get("email", "beta-user")
        device_id = test_request.get("device_id", "test-device")
            ...
            "threats": [
                {
                    ...
                    "threat_level": "medium",
                    "threat_reasons": ["Unusual network activity", "High CPU usage"]
                }
            ],
            "network_connections": [
                {
                    ...
                    "remote_port": 8080,
                    "threat_level": "low",
                    "activity_description": "Normal web traffic"
                }
            ],
            "risky_ports": [
                {
                    ...
                    "service": "SSH",
                    "threat_level": "medium",
                    "reason": "SSH port open to internet",
        ...
        # Store the test data
        latest_scan_data = test_scan_data
        ...
        latest_alerts_data = [
            {
                ...
                "description": "This is a test alert generated for beta testing",
                "severity": "medium",
                "timestamp": datetime.now().isoformat(),
                ...
                "isBlocked": False,
                "type": "test",
                "details": "Beta testing simulation"
            }
        ]
        
        logging.info(f"Simulated agent data for {email} on device {device_id}")
        
        return {
            "status": "success",
            "message": "Test agent data simulated successfully",
            "email": email,
            "device_id": device_id,
            "threats_simulated": len(test_scan_data["threats"]),
            "alerts_generated": len(latest_alerts_data),
            "timestamp": datetime.now().isoformat()
        }
        
    except Exception as e:
        logging.error(f"Failed to simulate agent data: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to simulate agent data: {str(e)}")
